...
@section('body')
    <form action="/register.php" method="post" class="bg-slate-100 rounded-md px-8 py-6 shadow border border-slate-300 max-w-md w-full flex flex-col space-y-3">
        <div class="flex flex-col">
            ...
            <input id="name" type="text" name="name" value="{{ $form->name }}" maxlength="25" autofocus autocomplete="name" required class="p-2 border border-slate-300 bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:ring-offset-0 focus:border-blue-500 rounded" />
            @error('name')
        ...
        <div class="flex flex-col">
            ...
            <input id="email" type="email" name="email" value="{{ $form->email }}" maxlength="250" autocomplete="email" required class="p-2 border border-slate-300 bg-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:ring-offset-0 focus:border-blue-500 rounded" />
            @error('email')
                <span class="text-sm text-red-500 pt-1">{{ $message }}</span>
            @enderror
        </div>
